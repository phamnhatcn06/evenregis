...
<body>
    ...
    <?php if (!empty($isDot1) || !empty($sportTeams)): ?>
        ...
        <?php else: ?>
            <?php
            // Đếm số đội theo từng nội dung để đánh số thứ tự khi có nhiều đội cùng nội dung
            $sportTeamCounts = array();
            foreach ($sportTeams as $t) {
                $key = !empty($t['sport_id']) ? $t['sport_id'] : $t['sport_name'];
                $sportTeamCounts[$key] = isset($sportTeamCounts[$key]) ? $sportTeamCounts[$key] + 1 : 1;
            }
            $sportTeamSeq = array();
            ...
            <?php foreach ($sportTeams as $tIdx => $team): ?>
                ...
                <div class="group-title">
                    <?php if ($showSportInItem): ?><?php echo CHtml::encode($team['sport_name']); ?><?php if ($hasMultipleTeams): ?> - Đội <?php echo $sportTeamSeq[$key]; ?><?php endif; ?><?php elseif ($hasMultipleTeams): ?>Đội <?php echo $sportTeamSeq[$key]; ?><?php endif; ?><?php if (!empty($team['team_name'])): ?><?php echo ($showSportInItem || $hasMultipleTeams) ? ': ' : ''; ?><?php echo CHtml::encode($team['team_name']); ?><?php endif; ?>
