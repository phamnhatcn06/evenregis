...
class GiixCrudCode extends CrudCode
{
    ...
    {
        // Start of CrudCode::generateActiveField code.
        if ($column->isForeignKey) {
            $relation = $this->findRelation($modelClass, $column);
            $relatedModelClass = $relation[3];

            /*
            return "echo \$form->dropDownListGroup(\$model, '{$column->name}', array(
                'widgetOptions' => array(
        ...
        if (
            ...
            || strtoupper($column->dbType) == 'TINYINT(1)'
            || strtoupper($column->dbType) == 'BIT'
            || strtoupper($column->dbType) == 'BOOL'
            || strtoupper($column->dbType) == 'BOOLEAN'
        ) {

            return "echo \$form->switchGroup(\$model, '{$column->name}',
				array(
					'widgetOptions' => array(
						'events'=>array(
            ...
            return "echo \$form->ckEditorGroup(\$model,'{$column->name}',
				array(
					'wrapperHtmlOptions' => array(
						/* 'class' => 'col-sm-5', */
					),
            ...
            strtolower($column->name) == 'icon'
            || strtolower($column->name) == 'img'
            || strtolower($column->name) == 'image'
            || strtolower($column->name) == 'avatar'
        ) {
            return "Yii::app()->clientScript->registerScript(\"image\", \"
                $('#selectedImg').click(function () {
                    var finder = new CKFinder();
                    finder.selectActionFunction = function (fileUrl) {
                        $('#{$modelClass}_{$column->name}').val(fileUrl);
                ...
                echo \$form->textFieldGroup(\$model, '{$column->name}', array('maxlength' => 255, 'prepend' => \$tmp,));";
        } else {
    ...
    'widgetOptions' => array('htmlOptions' => array(
      'class' => 'input w-full border mt-2'
                ...
                return "echo \$form->{$inputField}(\$model, '{$column->name}', array('maxlength' => {$column->size}, 'widgetOptions' => array('htmlOptions' => array(
      'class' => 'input w-full border mt-2'
    ))))";
        }
        ...
        if (!$column->isForeignKey) {
            if (
                ...
                return "array(
                            'type' => 'raw',
                            'name' => '{$column->name}',
                            'value' => (\$model->{$column->name} == 1) ? '<span class=\"badge badge-primary\">Đang hoạt động</span>' : '<span class=\"badge badge-danger\">Không kích hoạt</span>'
                        )";
            } else
                return "'{$column->name}'";
        } else {
    ...
    public function checkboxCondition($column)
    {
        $condition = false;
        if (
            strtoupper($column->dbType) == 'TINYINT(1)'
    ...
    public function imageCondition($column)
    {
        $condition = false;
        if ($column->type == 'string' && $column->size == 500) {
            $condition = true;
        }
        return $condition;
    }

    public function dateCondition($column)
    {
        $condition = false;
    ...
    public function editorCondition($column)
    {
        $condition = false;
    ...
    public function textAreaCondition($column)
    {
        $condition = false;
    ...
    public function sortCondition($column)
    {
        $condition = false;
        if (strtoupper($column->name) == 'SORT') {
            $condition = true;
        }
        return $condition;
    }
    ...
    public function generateGridViewColumn($modelClass, $column)
    {
        if (!$column->isForeignKey) {
            // Boolean or bit.
            if ($this->checkboxCondition($column)) {
                return "array(
                            'class' => 'booster.widgets.TbToggleColumn',
                            'toggleAction' => '$modelClass/toggle',
                            'name' => '$column->name',
                            'filter' => false,
                            'sortable' => false,
                            'htmlOptions' => array('style' => 'vertical-align: middle;text-align: center;width:5%'),
                            )";
            } else if ($this->sortCondition($column)) {

                return "array(
			'name'=>Yii::t('app','sort'),
            'value'=>'CHtml::textField(\"sort[sort][\$data->id]\",\$data->sort,array(\"size\"=>1))',
            'type'=>'raw',
            'htmlOptions'=>array('width'=>1,'style'=>'text-align : center'),
        )";
            } else if ($this->imageCondition($column)) {

                return "array(
            'name'=>'{$column->name}',
            'value' => '\$data->{$column->name} == \"\" ? \"\" : CHtml::image(\$data->{$column->name},\"\",array(\"style\"=>\"max-width:200px\") )',
            'type'=>'raw'
        )";
            } else if ($this->editorCondition($column) || $this->textAreaCondition($column))
                return "//'{$column->name}'";
