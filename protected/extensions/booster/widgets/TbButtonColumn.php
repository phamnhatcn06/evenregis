    ...
    protected function renderButton($id, $button, $row, $data)
    {
        ...
        if (isset($button['icon']) && $button['icon'] != '') {
            if (strpos($button['icon'], 'icon') === false && strpos($button['icon'], 'fa') === false) {
                $txtType = '';
                if (isset($button['options']['text-type']) && $button['options']['text-type'] != '') {
                    $txtType = 'txt-' . $button['options']['text-type'];
                }
                $button['icon'] = $txtType . ' mdi mdi-' . implode('mdi mdi-', explode(' ', $button['icon']));
            }

            echo CHtml::link('<i class="' . $button['icon'] . '"></i>&nbsp;&nbsp;'.$label, $url, $options);
        } else if (isset($button['imageUrl']) && is_string($button['imageUrl']) && $button['imageUrl'] != '') {
