...
//var_dump($widgetOptions);die;
    ...
    protected function horizontalGroup(&$fieldData, &$model, &$attribute, &$options)
    {
      ...
      self::addCssClass($groupOptions, 'form-group row');
        ...
        self::addCssClass($options['labelOptions'], 'col-3 col-form-label');
        if (isset($options['label'])) {
        ...
        $this->addCssClass($wrapperHtmlOptions, 'col-9');
    ...
    {
        $class = '';
        if (!empty($options['wrapperHtmlOptions']['class'])) {
            $class = $options['wrapperHtmlOptions']['class'];
        }

        echo '<div class="form-group ' . $class . '">';
        echo '<div class="border-text-box">';
        if (!empty($options['prepend']) || !empty($options['append']))
            $this->renderAddOnBegin($options['prepend'], $options['append'], $options['prependOptions']);
