...
class ChartJs extends CApplicationComponent
{
  ...
  public function init()
  {
    ...
    $cs->registerScriptFile($this->getAssetsUrl() . '/js/' . $jsFilename, CClientScript::POS_BEGIN);
    $cs->registerScriptFile($this->getAssetsUrl() . '/js/morris.min.js', CClientScript::POS_END);
    $cs->registerScriptFile($this->getAssetsUrl() . '/js/raphael-min.js', CClientScript::POS_END);
    $cs->registerCssFile($this->getAssetsUrl() . "/css/" . $cssFilename, '');
    $cs->registerCssFile($this->getAssetsUrl() . "/css/morris.css",'');
    }
