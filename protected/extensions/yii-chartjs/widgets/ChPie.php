...
class ChPie extends CWidget
{
  ...
  public function run()
  {
    ...
    $color = array();
    if ($this->datasets) {
      foreach ($this->datasets as $obj) {
        $data[] = $obj['value'];
        $label[] = $obj['label'];
        $color[] = "#".$obj['color'];
      }
    }
    $dataList = CJSON::encode($data);
    $labelList = CJSON::encode($label);
    $colorList = CJSON::encode($color);
    ...
    options: {
        ...
        legend: {
            ...
            var ctx = $(\"#" . $this->htmlId . "\").get(0).getContext(\"2d\");var chart = new Chart(ctx,config);"
    );
  }
