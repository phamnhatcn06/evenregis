 ...
 * @package system.web
 */
class CoreModule extends CModule {
    ...
    public function beforeControllerAction($controller, $action) {
        $https_modules = Params::ENABLE_HTTPS_MODULE_PATH;
        $modules = explode(',', $https_modules);
        $currentUrl = $this->fullUrl($_SERVER);
        $domain = $_SERVER['HTTP_HOST'];
        $ssl = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on') ? true : false;

        $urlMatch = false;
        foreach ($modules as $module) {
            $checkUrl = $domain . '/' . $module;
            if (strpos($currentUrl, $checkUrl) !== false) {
                $urlMatch = true;
                break;
            }
        }
        ...
        if (!$urlMatch && $ssl) {
            $nUrl = str_replace('https://', 'http://', strtolower($currentUrl));
            Yii::app()->request->redirect($nUrl);
        }
    ...
    public function isUrlMatch($url, $des_url) {
    ...
    public function fullUrl($s, $use_forwarded_host = false) {
        return $this->originUrl($s, $use_forwarded_host) . $s['REQUEST_URI'];
    }
