 ...
 * Run: php protected/yiic exporttestcases
 */
class ExportTestCasesCommand extends CConsoleCommand
{
    public function run($args)
    {
        ...
        // Save
        $outputPath = Yii::getPathOfAlias('application') . '/../docs/Test_cases.xlsx';
    ...
    private function getTestCases()
    {
        return array(
            ...
            array('id' => 'AUTH-001', 'module' => 'Xác thực & Phân quyền', 'description' => 'Đăng nhập thành công với JWT hợp lệ từ Portal', 'preconditions' => 'Portal đã cấp JWT token hợp lệ, user tồn tại trong hệ thống', 'steps' => '1. Portal redirect đến /auth/callback?token=<JWT> 2. Hệ thống decode JWT 3. Tạo session', 'expected' => 'Session được tạo, redirect về Dashboard; thông tin user_id, full_name, unit_code lưu vào session', 'priority' => 'Critical'),
            ...
            array('id' => 'AUTH-006', 'module' => 'Xác thực & Phân quyền', 'description' => 'Session hết hạn sau 30 phút không hoạt động', 'preconditions' => 'User đã đăng nhập, không thao tác 30 phút', 'steps' => '1. Chờ 30 phút 2. Thực hiện bất kỳ thao tác nào', 'expected' => 'Redirect về trang login/Portal, session bị xóa', 'priority' => 'High'),
            array('id' => 'AUTH-007', 'module' => 'Xác thực & Phân quyền', 'description' => 'Quyền Create (C) đúng theo JWT payload', 'preconditions' => 'JWT có "event": "1 1 1 1" (C=1)', 'steps' => '1. Đăng nhập 2. Truy cập action tạo sự kiện', 'expected' => 'Cho phép truy cập action create', 'priority' => 'Critical'),
            array('id' => 'AUTH-008', 'module' => 'Xác thực & Phân quyền', 'description' => 'Quyền Read (R) bị từ chối theo JWT payload', 'preconditions' => 'JWT có "event": "0 0 1 1" (R=0)', 'steps' => '1. Đăng nhập 2. Truy cập action list sự kiện', 'expected' => 'Trả về HTTP 403 "Forbidden"', 'priority' => 'Critical'),
            array('id' => 'AUTH-009', 'module' => 'Xác thực & Phân quyền', 'description' => 'Quyền Delete (D) bị từ chối đúng cách', 'preconditions' => 'JWT có "attendee": "1 1 1 0" (D=0)', 'steps' => '1. Đăng nhập 2. Thử xóa attendee', 'expected' => 'Nút Xóa bị ẩn trong view; nếu gọi trực tiếp URL trả về 403', 'priority' => 'Critical'),
            ...
            array('id' => 'AUTH-011', 'module' => 'Xác thực & Phân quyền', 'description' => 'Gọi API SSO /api/sso/me thành công sau đăng nhập', 'preconditions' => 'Đã đăng nhập thành công, token còn hiệu lực', 'steps' => '1. Hệ thống tự động gọi SSO API 2. Nhận profile', 'expected' => 'Profile được lưu vào localStorage phía client', 'priority' => 'High'),
            array('id' => 'AUTH-012', 'module' => 'Xác thực & Phân quyền', 'description' => 'Gọi API SSO thất bại (network timeout)', 'preconditions' => 'Mạng không ổn định', 'steps' => '1. Đăng nhập 2. API SSO timeout', 'expected' => 'Session vẫn được tạo từ JWT; user vẫn vào được hệ thống (degraded mode)', 'priority' => 'Medium'),
            ...
            array('id' => 'AUTH-016', 'module' => 'Xác thực & Phân quyền', 'description' => 'Admin HO có toàn quyền hệ thống', 'preconditions' => 'User có role=admin trong JWT', 'steps' => '1. Đăng nhập với role admin 2. Truy cập tất cả module', 'expected' => 'Tất cả CRUD đều được phép; không bị 403 ở bất kỳ action nào', 'priority' => 'Critical'),
            array('id' => 'AUTH-017', 'module' => 'Xác thực & Phân quyền', 'description' => 'HR chỉ thấy chức năng phê duyệt đăng ký', 'preconditions' => 'User có role=hr', 'steps' => '1. Đăng nhập với role hr 2. Vào module Competition', 'expected' => 'Chỉ có quyền Read; không thấy nút Create/Delete trong Competition nếu không được cấp', 'priority' => 'High'),
            array('id' => 'AUTH-018', 'module' => 'Xác thực & Phân quyền', 'description' => 'BTC Thể thao không vào được module Thi nghiệp vụ', 'preconditions' => 'User có role=sports_organizer; không có quyền competition', 'steps' => '1. Đăng nhập 2. Truy cập /admin/competition', 'expected' => 'Trả về 403 hoặc không hiển thị menu', 'priority' => 'High'),
            array('id' => 'AUTH-019', 'module' => 'Xác thực & Phân quyền', 'description' => 'Đại diện đơn vị không truy cập được trang admin HO', 'preconditions' => 'unit_account đăng nhập', 'steps' => '1. Đăng nhập tài khoản đơn vị 2. Truy cập trang admin', 'expected' => 'Từ chối truy cập, redirect về dashboard đơn vị', 'priority' => 'Critical'),

            // Module 2: Organizations
            array('id' => 'ORG-001', 'module' => 'Quản lý Đơn vị', 'description' => 'Tạo khu vực mới thành công', 'preconditions' => 'Admin đã đăng nhập', 'steps' => '1. Vào Quản lý Khu vực 2. Nhập mã "KV01", tên "Khu vực Hà Nội" 3. Lưu', 'expected' => 'Khu vực được tạo, xuất hiện trong danh sách', 'priority' => 'High'),
            ...
            array('id' => 'ORG-003', 'module' => 'Quản lý Đơn vị', 'description' => 'Tạo khu vực với tên rỗng', 'preconditions' => 'Admin đã đăng nhập', 'steps' => '1. Để trống trường Tên 2. Lưu', 'expected' => 'Lỗi validation "Tên khu vực là bắt buộc"', 'priority' => 'High'),
            ...
            array('id' => 'ORG-006', 'module' => 'Quản lý Đơn vị', 'description' => 'Tạo đơn vị mới với đầy đủ thông tin', 'preconditions' => 'Admin đã đăng nhập, khu vực tồn tại', 'steps' => '1. Nhập tên, mã đơn vị, chọn khu vực 2. Lưu', 'expected' => 'Đơn vị được tạo, liên kết đúng với khu vực', 'priority' => 'High'),
            ...
            array('id' => 'ORG-010', 'module' => 'Quản lý Đơn vị', 'description' => 'Cập nhật thông tin đơn vị', 'preconditions' => 'Đơn vị đã tồn tại', 'steps' => '1. Sửa tên đơn vị 2. Lưu', 'expected' => 'Thông tin cập nhật thành công, updated_at được gán', 'priority' => 'Medium'),
            array('id' => 'ORG-011', 'module' => 'Quản lý Đơn vị', 'description' => 'Tạo tài khoản đơn vị thành công', 'preconditions' => 'Đơn vị đã tồn tại, chưa có tài khoản', 'steps' => '1. Tạo tài khoản với username/password 2. Liên kết đơn vị', 'expected' => 'Tài khoản được tạo, liên kết 1-1 với đơn vị', 'priority' => 'High'),
            ...
            array('id' => 'ORG-017', 'module' => 'Quản lý Đơn vị', 'description' => 'Đổi mật khẩu tài khoản đơn vị', 'preconditions' => 'Đã đăng nhập', 'steps' => '1. Nhập mật khẩu cũ đúng 2. Nhập mật khẩu mới 3. Lưu', 'expected' => 'Mật khẩu được cập nhật, đăng nhập lại bằng mật khẩu mới thành công', 'priority' => 'Medium'),
            ...
            array('id' => 'REG-001', 'module' => 'Đăng ký Tham dự', 'description' => 'Admin tạo đợt đăng ký hợp lệ', 'preconditions' => 'Admin đã đăng nhập', 'steps' => '1. Nhập tên đợt, start_time, end_time, max_per_org=20 2. Lưu', 'expected' => 'Đợt đăng ký được tạo thành công', 'priority' => 'High'),
            array('id' => 'REG-002', 'module' => 'Đăng ký Tham dự', 'description' => 'Tạo đợt đăng ký với end_time < start_time', 'preconditions' => 'Admin đã đăng nhập', 'steps' => '1. Nhập end_time trước start_time 2. Lưu', 'expected' => 'Lỗi validation "Thời gian kết thúc phải sau thời gian bắt đầu"', 'priority' => 'High'),
            ...
            array('id' => 'REG-005', 'module' => 'Đăng ký Tham dự', 'description' => 'Đơn vị thấy đợt đăng ký đang mở', 'preconditions' => 'Đợt đăng ký is_active=1, trong thời hạn', 'steps' => '1. Đăng nhập tài khoản đơn vị 2. Xem danh sách đợt', 'expected' => 'Đợt đăng ký đang mở hiển thị, cho phép tạo phiếu', 'priority' => 'High'),
            ...
            array('id' => 'REG-007', 'module' => 'Đăng ký Tham dự', 'description' => 'Tạo phiếu đăng ký thành công', 'preconditions' => 'Đơn vị đã đăng nhập, đợt đang mở', 'steps' => '1. Chọn sự kiện 2. Chọn đợt đăng ký 3. Lưu nháp', 'expected' => 'Phiếu được tạo với status="draft", liên kết đúng org và period', 'priority' => 'Critical'),
            ...
            array('id' => 'REG-009', 'module' => 'Đăng ký Tham dự', 'description' => 'Thêm người tham dự vào phiếu draft', 'preconditions' => 'Phiếu có status="draft"', 'steps' => '1. Thêm người tham dự với tên, chức danh 2. Lưu', 'expected' => 'Người tham dự được thêm vào attendees, liên kết với registration_id', 'priority' => 'High'),
            ...
            array('id' => 'REG-013', 'module' => 'Đăng ký Tham dự', 'description' => 'Chỉnh sửa thông tin người tham dự khi draft', 'preconditions' => 'Phiếu status="draft", có attendee', 'steps' => '1. Sửa tên, chức danh 2. Lưu', 'expected' => 'Thông tin cập nhật thành công', 'priority' => 'High'),
            ...
            array('id' => 'REG-015', 'module' => 'Đăng ký Tham dự', 'description' => 'Nộp phiếu đăng ký', 'preconditions' => 'Phiếu draft có ít nhất 1 attendee', 'steps' => '1. Click "Nộp đăng ký" 2. Xác nhận', 'expected' => 'Status chuyển thành "submitted", submitted_at được gán, thông báo thành công', 'priority' => 'Critical'),
            ...
            array('id' => 'REG-021', 'module' => 'Đăng ký Tham dự', 'description' => 'HR phê duyệt phiếu đăng ký (approve)', 'preconditions' => 'Phiếu status="submitted"', 'steps' => '1. HR chọn phiếu 2. Click "Phê duyệt"', 'expected' => 'Status chuyển thành "approved", reviewed_by và reviewed_at được gán', 'priority' => 'Critical'),
            array('id' => 'REG-022', 'module' => 'Đăng ký Tham dự', 'description' => 'HR từ chối phiếu đăng ký kèm lý do', 'preconditions' => 'Phiếu status="submitted"', 'steps' => '1. HR click "Từ chối" 2. Nhập lý do 3. Xác nhận', 'expected' => 'Status chuyển thành "rejected", rejection_reason được lưu', 'priority' => 'Critical'),
            array('id' => 'REG-023', 'module' => 'Đăng ký Tham dự', 'description' => 'Từ chối phiếu mà không nhập lý do', 'preconditions' => 'Phiếu status="submitted"', 'steps' => '1. Click "Từ chối" 2. Để trống lý do 3. Xác nhận', 'expected' => 'Lỗi "Vui lòng nhập lý do từ chối"', 'priority' => 'High'),
            ...
            // Module 4: Attendees
            array('id' => 'ATT-001', 'module' => 'Người tham dự', 'description' => 'Admin chỉnh sửa thông tin attendee sau phê duyệt', 'preconditions' => 'Phiếu đã approved, attendee tồn tại', 'steps' => '1. Admin mở attendee 2. Sửa tên/chức danh 3. Lưu', 'expected' => 'Thông tin cập nhật, updated_at gán mới; audit log ghi lại', 'priority' => 'High'),
            ...
            array('id' => 'ATT-003', 'module' => 'Người tham dự', 'description' => 'QR token là duy nhất (không trùng giữa các attendee)', 'preconditions' => 'Nhiều attendee trong hệ thống', 'steps' => '1. Tạo 100 attendee 2. Kiểm tra qr_token', 'expected' => 'Tất cả qr_token khác nhau (UNIQUE constraint)', 'priority' => 'Critical'),
            ...
            array('id' => 'ATT-005', 'module' => 'Người tham dự', 'description' => 'Gán vai trò cho người tham dự', 'preconditions' => 'Attendee đã approved, role tồn tại', 'steps' => '1. Admin chọn attendee 2. Gán role "Trưởng đoàn"', 'expected' => 'Bản ghi trong attendee_roles được tạo', 'priority' => 'High'),
            ...
            array('id' => 'ATT-007', 'module' => 'Người tham dự', 'description' => 'Gán trưởng đoàn cho đơn vị', 'preconditions' => 'Đơn vị có nhiều attendee', 'steps' => '1. Admin chọn 1 attendee làm trưởng đoàn 2. Lưu', 'expected' => 'is_team_lead=1 cho attendee đó; các attendee khác vẫn is_team_lead=0', 'priority' => 'High'),
            ...
            // Module 5: Badges
            array('id' => 'BAD-001', 'module' => 'Thẻ tham dự', 'description' => 'Tạo thẻ cho 1 attendee', 'preconditions' => 'Attendee đã approved, có đầy đủ thông tin', 'steps' => '1. Chọn attendee 2. Click "Tạo thẻ"', 'expected' => 'File ảnh thẻ được tạo (85.60×53.98mm, 300dpi), badge_generated=1', 'priority' => 'Critical'),
            array('id' => 'BAD-002', 'module' => 'Thẻ tham dự', 'description' => 'Tạo thẻ theo lô (batch)', 'preconditions' => 'Nhiều attendee đã approved', 'steps' => '1. Chọn tất cả attendee 2. Tạo thẻ hàng loạt', 'expected' => 'Tất cả thẻ được tạo; báo cáo thành công/thất bại', 'priority' => 'High'),
            array('id' => 'BAD-003', 'module' => 'Thẻ tham dự', 'description' => 'Thẻ chứa QR code đúng qr_token', 'preconditions' => 'Attendee có qr_token', 'steps' => '1. Tạo thẻ 2. Quét QR', 'expected' => 'URL trong QR dẫn đến /frontend/attendee/view?token=<qr_token> của đúng attendee', 'priority' => 'Critical'),
            array('id' => 'BAD-004', 'module' => 'Thẻ tham dự', 'description' => 'Kích thước ảnh thẻ đúng chuẩn CR80', 'preconditions' => 'Thẻ vừa được tạo', 'steps' => '1. Tạo thẻ 2. Kiểm tra kích thước file', 'expected' => 'Ảnh có tỉ lệ đúng 85.60:53.98mm, DPI=300 (tương đương 1013×638 pixel)', 'priority' => 'High'),
            ...
            array('id' => 'BAD-008', 'module' => 'Thẻ tham dự', 'description' => 'Tái tạo thẻ (regenerate) sau khi sửa thông tin', 'preconditions' => 'Thẻ đã tạo, admin sửa tên attendee', 'steps' => '1. Sửa tên 2. Tái tạo thẻ', 'expected' => 'Thẻ mới phản ánh tên mới; file cũ bị ghi đè', 'priority' => 'Medium'),
            ...
            array('id' => 'SPT-004', 'module' => 'Thi Thể thao', 'description' => 'Upload file điều lệ thi đấu (document)', 'preconditions' => 'Môn thể thao tồn tại', 'steps' => '1. Upload file PDF điều lệ', 'expected' => 'File lưu thành công, document path cập nhật', 'priority' => 'Medium'),
            array('id' => 'SPT-005', 'module' => 'Thi Thể thao', 'description' => 'Tạo đội thi đấu cho đơn vị', 'preconditions' => 'Môn thể thao tồn tại, đơn vị tồn tại', 'steps' => '1. Tạo đội "Đội Bóng đá HN01" cho môn Bóng đá 2. Liên kết đơn vị', 'expected' => 'Đội được tạo với organization_id đúng', 'priority' => 'High'),
            ...
            array('id' => 'SPT-007', 'module' => 'Thi Thể thao', 'description' => 'Thêm thành viên vào đội', 'preconditions' => 'Đội và attendee tồn tại', 'steps' => '1. Thêm attendee vào đội 2. Gán số áo, vị trí', 'expected' => 'Bản ghi sport_team_members được tạo', 'priority' => 'High'),
            ...
            array('id' => 'SPT-015', 'module' => 'Thi Thể thao', 'description' => 'Cập nhật kết quả trận thắng-thua', 'preconditions' => 'Trận status="ongoing"', 'steps' => '1. Nhập score_a="3", score_b="1" 2. Chọn winner=Đội A', 'expected' => 'Bản ghi sport_match_results tạo/cập nhật; trận status="completed"', 'priority' => 'High'),

            // Module 7: Competition
            array('id' => 'CMP-001', 'module' => 'Thi Nghiệp vụ', 'description' => 'Tạo cuộc thi nghiệp vụ mới', 'preconditions' => 'BTC Nghiệp vụ đã đăng nhập', 'steps' => '1. Nhập tên cuộc thi, prefix="NV", has_qualification=1 2. Lưu', 'expected' => 'Cuộc thi được tạo thành công', 'priority' => 'High'),
            array('id' => 'CMP-002', 'module' => 'Thi Nghiệp vụ', 'description' => 'Tạo vòng thi cho cuộc thi', 'preconditions' => 'Cuộc thi đã tạo', 'steps' => '1. Thêm "Vòng loại" round_order=1 2. Thêm "Chung kết" round_order=2', 'expected' => '2 vòng thi được tạo, liên kết đúng competition_id', 'priority' => 'High'),
            ...
            array('id' => 'MEA-002', 'module' => 'Bữa ăn & Tiệc', 'description' => 'Trưởng đoàn xem danh sách thành viên', 'preconditions' => 'is_team_lead=1, đơn vị có nhiều attendee', 'steps' => '1. Đăng nhập trưởng đoàn 2. Xem danh sách', 'expected' => 'Chỉ hiển thị attendee cùng đơn vị với trưởng đoàn', 'priority' => 'High'),
            array('id' => 'MEA-003', 'module' => 'Bữa ăn & Tiệc', 'description' => 'Báo cắt ăn cho từng người', 'preconditions' => 'Trưởng đoàn đã đăng nhập, bữa chưa qua cutoff', 'steps' => '1. Chọn thành viên A 2. Báo cắt bữa sáng', 'expected' => 'Bản ghi meal_cutoffs được tạo cho attendee A', 'priority' => 'High'),
            ...
            array('id' => 'BAN-001', 'module' => 'Bữa ăn & Tiệc', 'description' => 'Tạo sự kiện tiệc', 'preconditions' => 'BTC Tiệc đã đăng nhập', 'steps' => '1. Nhập tên "Tiệc tối khai mạc", thời gian, địa điểm 2. Lưu', 'expected' => 'Sự kiện tiệc được tạo', 'priority' => 'High'),
            array('id' => 'BAN-002', 'module' => 'Bữa ăn & Tiệc', 'description' => 'Thiết lập sơ đồ bàn', 'preconditions' => 'Sự kiện tiệc tồn tại', 'steps' => '1. Tạo 30 bàn với capacity=10 2. Gán vị trí (pos_x, pos_y)', 'expected' => '30 bàn được tạo trong banquet_tables', 'priority' => 'High'),
            ...
            array('id' => 'QR-007', 'module' => 'QR Code Public', 'description' => 'Xem lịch thi nghiệp vụ cá nhân', 'preconditions' => 'Attendee đã đăng ký thi nghiệp vụ', 'steps' => '1. Quét QR của attendee 2. Xem lịch thi', 'expected' => 'Chỉ hiển thị cuộc thi và vòng thi mà attendee đăng ký', 'priority' => 'High'),
