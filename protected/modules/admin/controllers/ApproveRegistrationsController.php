...
class ApproveRegistrationsController extends AdminController
{
    ...
    public function actionSendEmail()
    {
        ...
        // Tự động tìm email người nhận nếu chưa nhập (mặc định cswm@muongthanh.vn cho gửi test)
        if (empty($recipientEmail)) {
            if (!empty($model->submitted_by) && filter_var($model->submitted_by, FILTER_VALIDATE_EMAIL)) {
                $recipientEmail = $model->submitted_by;
            } else {
                $recipientEmail = 'cswm@muongthanh.vn';
            }
        }

        if (empty($recipientEmail) || !filter_var($recipientEmail, FILTER_VALIDATE_EMAIL)) {
            echo CJSON::encode(array('success' => false, 'error' => 'Địa chỉ email người nhận không hợp lệ. Vui lòng kiểm tra lại.'));
            Yii::app()->end();
        }
        ...
        if ($model->property_id && empty($model->property_name)) {
            $property = Properties::fetchFromApi($model->property_id);
        ...
        // Load Competition Registrations (Đợt 2 - Nghiệp vụ)
        $competitionRegistrations = array();
        ...
        // Load Sport Teams (Đợt 1 - Thể thao)
        ...
        try {
            $sent = MyHelper::sendMail($recipientEmail, $subject, 'registration_confirmation', $data);
            if ($sent) {
                echo CJSON::encode(array(
                    'success' => true,
                    'message' => 'Đã gửi email xác nhận thành công đến ' . CHtml::encode($recipientEmail) . '.',
                ));
            } else {
                echo CJSON::encode(array('success' => false, 'error' => 'Không thể gửi email. Vui lòng kiểm tra lại cấu hình mail.'));
            }
        } catch (Exception $e) {
            Yii::log('Send confirmation email error: ' . $e->getMessage(), 'error', 'application.controllers.ApproveRegistrationsController');
            echo CJSON::encode(array('success' => false, 'error' => 'Lỗi gửi mail: ' . $e->getMessage()));
        }
