...
class ReportsController extends AdminController
{
    ...
    public function actionReportByHotel()
    {
        ...
        // 1. Sport Athletes - người có đăng ký môn thể thao
        $teamParams = array('event_id' => $selectedEventId, 'per_page' => 1000);
        if (!$isHO && $userPropertyId) {
            $teamParams['property_id'] = $userPropertyId;
        }
        ...
        // 2. Competition Contestants - người đăng ký thi nghiệp vụ
        ...
        // 3. Beauty Contestants - người thi Miss
        $contests = BeautyContests::getApiDataProvider(array('event_id' => $selectedEventId), 100)->getData();
        foreach ($contests as $contest) {
        ...
        // 4. Talent Entries - tiết mục văn nghệ
        $shows = TalentShows::getApiDataProvider(array('event_id' => $selectedEventId), 100)->getData();
        $showIds = array();
        $showNames = array();
        foreach ($shows as $show) {
        ...
        $this->render('report_by_hotel', array(
            'isHO' => $isHO,
            'user' => $user,
            'eventsList' => $eventsList,
            'selectedEventId' => $selectedEventId,
     ...
     */
    public function actionExportByHotel($event_id = null, $property_id = null, $type = null)
    {
        $user = AuthHandler::getUser();
        if (!$user) {
            throw new CHttpException(403, 'Bạn cần đăng nhập để xuất báo cáo.');
        ...
        $exportType = $type; // sport, competition, beauty, talent, all
