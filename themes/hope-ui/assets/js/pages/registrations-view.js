var RegistrationView = (function() {
    ...
    function loadAttendeesForAddTalentMember(entryId) {
        ...
        .then(function(entryRes) {
            ...
            // Fetch available attendees từ registration hiện tại
    ...
    function renderTalentMemberLists() {
        ...
        talentMemberAllAttendees.forEach(function(att) {
            ...
            // Đánh dấu người có thể copy hồ sơ từ đăng ký trước
            if (att.can_copy_from_previous) {
                badges += ' <span class="badge bg-info ms-1" title="Có hồ sơ từ đăng ký trước - sẽ tự động duyệt"><i class="fa fa-check-circle"></i> Có HS</span>';
            } else if (att.has_documents) {
                badges += ' <span class="badge bg-success ms-1" title="Đã có đủ hồ sơ"><i class="fa fa-check"></i></span>';
            }

            if (att.has_talent_role) {
                badges += ' <span class="badge bg-primary ms-1" title="Đã có vai trò văn nghệ"><i class="fa fa-music"></i></span>';
            }
        ...
        Promise.all(promises)
            .then(function(results) {
                ...
                if (failed.length > 0) {
                    ...
                    if (copiedCount > 0) {
                        msg += '. Đã sao chép hồ sơ cho ' + copiedCount + ' người từ đăng ký trước.';
                    }
                    Toast.success(msg);
                    location.reload();
                }
