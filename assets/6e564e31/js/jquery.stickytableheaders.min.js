!function (a, b) {
    ...
    function c(c, g) {
        ...
        h.$el = a(c), h.el = c, h.id = e++, h.$el.bind("destroyed", a.proxy(h.teardown, h)), h.$clonedHeader = null, h.$originalHeader = null, h.isSticky = !1, h.hasBeenSticky = !1, h.leftOffset = null, h.topOffset = null, h.init = function () {
            h.options = a.extend({}, f, g), h.$el.each(function () {
                var b = a(this);
            ...
            h.$scrollableArea.on("scroll." + d, h.toggleHeaders), h.isWindowScrolling() || (a(b).on("scroll." + d + h.id, h.setPositionValues), a(b).on("resize." + d + h.id, h.toggleHeaders)), h.$scrollableArea.on("resize." + d, h.toggleHeaders), h.$scrollableArea.on("resize." + d, h.updateWidth)
        }, h.unbind = function () {
            h.$scrollableArea.off("." + d, h.toggleHeaders), h.isWindowScrolling() || (a(b).off("." + d + h.id, h.setPositionValues), a(b).off("." + d + h.id, h.toggleHeaders)), h.$scrollableArea.off("." + d, h.updateWidth), h.$el.off("." + d), h.$el.find("*").off("." + d)
            ...
            h.$el && h.$el.each(function () {
                ...
                i && j ? (b = e.left - g + h.options.leftOffset, h.$originalHeader.css({
                    ...
                    left: b,
                    "z-index": 1
                }), h.isSticky = !0, h.leftOffset = b, h.topOffset = d, h.$clonedHeader.css("display", ""), h.setPositionValues(), h.updateWidth()) : h.isSticky && (h.$originalHeader.css("position", "static"), h.$clonedHeader.css("display", "none"), h.isSticky = !1, h.resetWidth(a("td,th", h.$clonedHeader), a("td,th", h.$originalHeader)))
            })
        }, h.isWindowScrolling = function () {
            return h.$scrollableArea[0] === b
        }, h.setPositionValues = function () {
            var c = a(b).scrollTop(), d = a(b).scrollLeft();
            !h.isSticky || 0 > c || c + a(b).height() > a(document).height() || 0 > d || d + a(b).width() > a(document).width() || h.$originalHeader.css({
                top: h.topOffset - (h.isWindowScrolling() ? 0 : c),
                left: h.leftOffset - (h.isWindowScrolling() ? 0 : d)
            })
        ...
        }, h.getWidth = function (b) {
            var c = [];
            return b.each(function (b) {
                var d, e = a(this);
                d = "border-box" === e.css("box-sizing") ? e.outerWidth() : e.width(), c[b] = d
            }), c
        ...
        }, h.updateOptions = function (b) {
            h.options = a.extend({}, f, b), h.updateWidth(), h.toggleHeaders()
        }, h.init()
    }
